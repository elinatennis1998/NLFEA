...
AbaqusInputReader
AlgoType = [0; 1; 0];
MatTypeTable = [MatTypeTable; zeros(1,nummat)];
...
temptext = strrep(temptext, oldtype, headerData{2,2});
...
for item = 1:numBC
    ...
    for j = 1:Block %size(headerData,2)
        if strcmp(BCtext,headerData{2,j}) && strcmp('*Nset',headerData{1,j})
            ...
            [Data{foundBC,1},~] = UpdateNodeSet(maxel,BCmat,RegionOnElement,ElementsOnNodeNum,...
                 ElementsOnNode,ElementsOnNodeDup,NodeBCholder{item},length(NodeBCholder{item}));
            [NodeBCholder{item},~] = UpdateNodeSet(maxel,BCmat,RegionOnElement,ElementsOnNodeNum,...
                 ElementsOnNode,ElementsOnNodeDup,NodeBCholder{item},length(NodeBCholder{item}));
            break
        end
    end
    ...
    for mat1 = 1:mat2
        ...
        [NodesOnElement,RegionOnElement,nen,numel,nummat,MatTypeTable,MateT] = ...
         FormCZ(SurfacesIi,NodesOnElement,RegionOnElement,Coordinates,numSIi,nen_bulk,2,numel,nummat,6, ...
                22,0,[100000 3 0.2],MatTypeTable,MateT);
        end
    end
end
