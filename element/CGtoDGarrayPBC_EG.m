...
xlM = xl(1:ndm,nelL+nelR+7:nelL+nelR+6+ndm); %for MateTypeTable of 14 columns (quad)
%  xlM = xl(1:ndm,15:16); 
if isw > 1 && isw ~= 53 %Modificaion of 4/17/2019
ulL = ul(1:ndf,1:nelL);
ulR = ul(1:ndf,nelL+1:nelL+nelR);
ulM = ul(1:ndf,nelL+nelR+7:nelL+nelR+6+ndm); %for MateTypeTable of 14 columns (quad)
end
matepropL = MateT(maL,:);
matepropR = MateT(maR,:);