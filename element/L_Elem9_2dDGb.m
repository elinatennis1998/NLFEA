...
if neper_abaqus ==1 %nonconforming mesh with neper
         if dx == xlM(1,1) 
         ...
         elseif dy == xlM(2,2) 
            nodeAR = SurfacesI(inter,2);
            nodeBR = SurfacesI(inter,1);
            nodeAL = SurfacesI(inter,4);
            nodeBL = SurfacesI(inter,3);
         else
            fprintf('There is error in the InterConn2D2PBC condition');   
         end

elseif neper_abaqus==2  % reverse of case 1 (e.g abaqus)
         if dx == xlM(1,1) 
            nodeAR = SurfacesI(inter,2);
            nodeBR = SurfacesI(inter,1);
            nodeAL = SurfacesI(inter,4);
...
else  %when other components of Surfaces are not provided
        nodeAR = -1;
        nodeBR = -1;
        nodeAL = -1;
        nodeBL = -1;
