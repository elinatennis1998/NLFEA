...
%         end %je
    ...
    case 50
        ...
        for je = 1:lint
            ...
            epsil = (Bmat*ulres(1:nel*ndf) - cvec);
            sigma = Dmat*epsil;
            
            ElemE = ElemE + c1*epsil;
            ElemS = ElemS + c1*[sigma; displac];
            ElemV = ElemV + c1*[1; sigma'*epsil];
    ...
    case 52
        ...
        ElemE = zeros(3,1);
        ElemV = zeros(2,1);
        ElemDisp = zeros(6,1); %Average grain displacement 
        Nmat = zeros(2,ndf*nel);
        ...
        for je = 1:lint
            ...
            ElemV = ElemV + c1*[1; 0];
            %Added 6/25/2019 
            %Not sure if right. see notebook notes
            %Average Displacement based on applied load
%             t1 = [xs(:,1); 0];
%             [tm1, tu1] = VecNormalize(t1);
%             t2 = [0; 0; 1];
%             tm2 = 1;
...
%                      0 nLy nLx];
%             nvec = [nLx; nLy];
%             ElemDisp = ElemDisp(1:2) + c1*xint*eRVE(1:2)*nvec;
%             ElemDisp = ElemDisp(1:4) + reshape(c1*xint*eRVE(1:2),4,1);
