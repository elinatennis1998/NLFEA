...
[~,back] = strtok(back, ' ');
[etype,~] = strtok(back, ' ');
headerData{2,Block}=etype(6:end);

% Determine number of nodes per element, nen
nen = AbaqusNEL(etype(6:end),ndm);
...
line=fgetl(fid);
    ...
    if length(line) > 1 && strcmp(line(1:2),'**') == 0 % not a comment line
        ...
        if strcmp(front,'*Nset')
            ...
            [middle,back]=strtok(rem,',');
            headerData{2,Block}=middle(7:end); % name of set
...
Stuff=cell2mat(InputText); % Convert to numerical array from cell
        ...
        elseif strcmp(front,'*Elset')
            ...
            headerData{1,Block}=front; % type of set
            [middle,back]=strtok(rem,',');
            headerData{2,Block}=middle(8:end); % name of set
...
Stuff=cell2mat(InputText); % Convert to numerical array from cell
        ...
        elseif strcmp(front,'*Solid Section')
            ...
            Sblock = Sblock + 1;
            [middle,back]=strtok(rem,',');
            SectData{1,Sblock}=middle(8:end); % set name
            SectData{2,Sblock}=back(12:end); % material type
        ...
        elseif strcmp(front,'*Cohesive Section')
            ...
            Sblock = Sblock + 1;
            [middle,back]=strtok(rem,',');
            [back,last]=strtok(back,',');
            SectData{1,Sblock}=middle(8:end); % set name
            SectData{2,Sblock}=back(11:end); % material type
            % Any lines after this with numerical data are currently
            % ignored.
        ...
        elseif strcmp(front,'*Element')
            ...
            Block = Block + 1;
            headerData{1,Block}=front;
            headerData{2,Block}=rem(8:end);

            % Determine number of nodes per element, nen
            nel = AbaqusNEL(back(8:end),ndm);
...
Stuff=cell2mat(InputText); % Convert to numerical array from cell
            ...
            for j = 3:Block-1
                ...
                if strcmp(settype,front) && strcmp(middle(7:end),setname)
...
while notdone && feof(fid) == 0
    ...
    if strcmp(line(1:2),'**') == 0 && strcmp(line,'** ----------------------------------------------------------------') == 0 % not a comment
        ...
        if strcmp(front,'*Material')
            ...
            MateData{2,Mblock}=middle(7:end); % material type
