...
for mg = 1:nummat
    errchk = find(RegionOnElement(1:numel)==mg);
    if size(errchk,1) > 0
        errchk = find(MatTypeTable(1,1:nummat)==mg);
        if size(errchk,1) == 0
            errmsg = ['error in RegionOnElement: global material mg=' num2str(mg) 'does not have a corresponding local material in MatTypeTable'];
            error(errmsg)
        end
    end
end
...
for ma = 1:nummat
    ...
    nonlin = MatTypeTable(3,ma); % Lin/Nonlin flag
    mateprop = MateT(ma,:);
    
    if mg < 1 || mg > nummat || (ismember(nonlin,[0 1]) == 0)
