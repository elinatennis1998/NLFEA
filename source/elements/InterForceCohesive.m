...
% 08/15/2016
%
% Evaluate interactive force for cohesive type, called by NL_Elem34_3dBoth;
...
u1 = (ulA(1:3,1:nelA))*shl;
u2 = (ulB(1:3,1:nelB))*shl2;

IntFor = JxXif*kIF*(u1 - u2);

% Copied from the single elements NL_Elem26_3dMG and NL_Elem33_3dM
ElemF(1:nel1*ndf) = ElemF(1:nel1*ndf) - c1*(Nmat1'*JxXif*kIF*u1);
ElemK(1:nel1*ndf,1:nel1*ndf) = ElemK(1:nel1*ndf,1:nel1*ndf) ...
    + c1*(JxXif*Nmat1'*kIF*Nmat1);
...
ulB_n = ul1_n;
JxXif = JxX2;
u2 = (ulA(1:3,1:nelA))*shl;

ElemF(nel1*ndf+1:nel*ndf) = ElemF(nel1*ndf+1:nel*ndf) - c2*(Nmat2'*JxXif*kIF*u2);
ElemK(nel1*ndf+1:nel*ndf,nel1*ndf+1:nel*ndf) = ElemK(nel1*ndf+1:nel*ndf,nel1*ndf+1:nel*ndf) ...
    + c2*(JxXif*Nmat2'*kIF*Nmat2);


%% Projection terms
% 1-2
    
u2 = (ul2(1:3,1:nel2))*shl;
...
ElemF(1:nel1*ndf) = ElemF(1:nel1*ndf) - c2*(-Nmat1p'*JxX2*kIF*u2);

ElemK(1:nel1*ndf,nel1*ndf+1:nel*ndf) = ElemK(1:nel1*ndf,nel1*ndf+1:nel*ndf) ...
    - c2*(Nmat1p'*JxX2*kIF*Nmat2);


% 2-1
    
u1 = (ul1(1:3,1:nel1))*shl;
...
ElemF(nel1*ndf+1:nel*ndf) = ElemF(nel1*ndf+1:nel*ndf) - c1*(-Nmat2p'*JxX1*kIF*u1);

ElemK(nel1*ndf+1:nel*ndf,1:nel1*ndf) = ElemK(nel1*ndf+1:nel*ndf,1:nel1*ndf) ...
    - c1*(Nmat2p'*JxX1*kIF*Nmat1);
