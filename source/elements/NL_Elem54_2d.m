...
% Mixed method
% Implementation copied from NL_Elem53_2d
...
forceelast = 0;1; % force elastic value of tau
...
%         end
        ...
        [tauL,history1,intb,CnL] = Tau2d54(xlintL,xlL,ulL,ul_nL,history,matepropL,plasmodel,plasversion,nelL,nen,ndf,forceelast);
        ...
        [tauR,history1,intb,CnR] = Tau2d54(xlintR,xlR,ulR,ul_nR,history,matepropR,plasmodel,plasversion,nelR,nen,ndf,forceelast);
        ...
        for ie = 1:lint            
            ...
            Srot = mm10_RT2RVE(g);
            CnR3 = eye(6);
            CnR3([1 2 4],[1 2 4]) = CnR;
            CnR3 = Srot*CnR3*Srot';
            CnRrot = CnR3([1 2 4],[1 2 4]);
            CnL3 = eye(6);
            CnL3([1 2 4],[1 2 4]) = CnL;
            CnL3 = Srot*CnL3*Srot';
            CnLrot = CnL3([1 2 4],[1 2 4]);
            end
        ...
        hr(nh3+12:nh3+17) = [sort(eig(CnLrot)); sort(eig(CnRrot))];
%             if iter == 1 && (elem == n^2/4+1 || elem == n^2/4+2)
%                 CnL
%                 eigCnL=eig(CnL)
%                 CnR
%                 eigCnR=eig(CnR)
...
%                     D22 = J2Radial6thOrder(eps3d,ep_n,beta_n,a_n,mu,bulk,Khard,Hhard,sigy); %Analytical 6th order tensor
%                     dmatL1 = D22([1 2 4 7 8 10 19 20 22],[1 2 4]);
                    else
                        dmatL1 = zeros(9,3);
                    end
                end
...
%                     D22 = J2Radial6thOrder(eps3d,ep_n,beta_n,a_n,mu,bulk,Khard,Hhard,sigy);
...
%%
    case 26 % Element stresses: output stability tensors
        ...
        ep =   [hr(nh3+8) hr(nh3+9)
                hr(nh3+10) hr(nh3+11)];
        eigCnL = hr(nh3+12:nh3+14);
        eigCnR = hr(nh3+15:nh3+17);
