...
switch isw %Task Switch
    ...
    case 1
        ...
        nh1 = 5*3*2; % 5 variables, 3 int points, 2 triangles
...
%%
    ...
    % For separate bubble types on T and B
            ...
            gamL = 1/2*eye(3);
            gamR = 1/2*eye(3);
            ep = rp;1000;pencoeff*eye(3)*max(muL,muR)/h;
            else
            % RFB
            end
    ...
    %%            
                if tn >= 0 % tension
                  ...
                  elseif dmax > 0 && rp*dmax <= normtvec && normtvec <= rp*dmax + psik %unloading
    ...
    %                 dmax = max(dmax,deq);
                  ...
                  elseif rp*dmax + psik < normtvec % damage
    ...
    %                 dmax = max(dmax,deq);
                ...
                ElemKLL = ElemKLL + c1*rp*NmatL'*dddt*bnAdN1 + c1*bnAdN1'*dddu*NmatL;
                ElemKLR = ElemKLR + c1*rp*NmatL'*dddt*bnAdN2 - c1*bnAdN1'*dddu*NmatR;
                ElemKRL = ElemKRL - c1*rp*NmatR'*dddt*bnAdN1 + c1*bnAdN2'*dddu*NmatL;
                ElemKRR = ElemKRR - c1*rp*NmatR'*dddt*bnAdN2 - c1*bnAdN2'*dddu*NmatR;

                ElemKLL = ElemKLL - c1*bnAdN1'*dddt*bnAdN1;
                ElemKLR = ElemKLR - c1*bnAdN1'*dddt*bnAdN2;
                ElemKRL = ElemKRL - c1*bnAdN2'*dddt*bnAdN1;
                ElemKRR = ElemKRR - c1*bnAdN2'*dddt*bnAdN2;
                ...
                damhr = nh2-1+(ll-1)*5;
                dmaxhr = nh2-1+(ll-1)*5+4;
                dbonhr = nh2-1+(ll-1)*5+5;
                hr(damhr+1) = dvec(1);
                hr(damhr+2) = dvec(2);
                hr(damhr+3) = dvec(3);
                hr(dmaxhr) = dmax;
        ...
        for intt = 1:nil %integrate on left domain
            ...
            for l = 1:lint
                ...
                if nelL == 4 || nelL == 10
                  ...
                  PxyL = shgtt(xlL(:,1:nelL),nelL,shldL,shls,nelL,0,0,be);
                else
                  [shlL,shldL,shls,be] = shlb([rL sL tL],nelL,nelL,0,0);
                  PxyL = shgb(xlL(:,1:nelL),nelL,shldL,shls,nelL,0,0,be);
                end
                QxyL = PxyL;
                ...
                if nelR == 4 || nelR == 10
                  ...
                  PxyR = shgtt(xlR(:,1:nelR),nelR,shldR,shls,nelR,0,0,be);
                else
                  [shlR,shldR,shls,be] = shlb([rR sR tR],nelR,nelR,0,0);
                  PxyR = shgb(xlR(:,1:nelR),nelR,shldR,shls,nelR,0,0,be);
                end
                QxyR = PxyR;
