...
%
% Added least squares term from VBAF06.pdf, page 8 eq. (27).
% Results in "artificial" diffusion of sorts, but it controls the
% oscillations from the two peaks coming together.

% Set Material Properties
matmodel = @simple3x1;
mprops = [mateprop(7) mateprop(9)];

% LS beta factor; 1 = GLS, 0 = LS
LSBeta = .5;
    ...
    case 3 %Compute Stiffness and Residual
        ...
        for ll = 1:lint                    
            ...
            ElemK = ElemK + c1*(coeffm*(Nmat'*rho*Nmat) + coeffk*(Bmat'*Dmat*Bmat + Nmat'*v_d*Bmat + (Nmat'*Bmat)*(ulres*(dv_d'*Nmat))...
                          + Bmat'*v_d^2*tstep/(1+LSBeta)*Bmat)); %LS term
            ElemF = ElemF - c1*(Nmat'*rho*Nmat*vlres      + (Bmat'*Dmat*Bmat + Nmat'*v_d*Bmat+ Bmat'*v_d^2*tstep/(1+LSBeta)*Bmat)*ulres);
