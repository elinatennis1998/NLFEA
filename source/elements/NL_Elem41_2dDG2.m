...
% Unequal tension/shear limit states
...
%             [dvec,dddt,dmax,dstate,bonddbond2,dbond,chatter] = RadialCZME(tvtr,jumpu,dmax,dvec, ...
%     nvec,sigmax,Hc,dc,rp,dstate,iter,elast_init,itchat,chatter,bonddbond1,dtol,ndm);
        dddt2=zeros(2);
            [dvec3,dddt3,dmax3,dstate3,bonddbond23,dbond3,chatter3] = GeneralCZM(tvtr,jumpu,dmax,dvec, ...
...
%         for j = 1:props.num_hard
            for k = 1:2
                ...
                hi = hhh*abs(tvtr(k) + rp*jumpu(k));
                end
                A(k) = A(k) + hi;
            [Ri,~,~,~,~,~,~] = GeneralCZM(A,jumpu,dmax,dvec, ...
    nvec,[sigmax*dc/2 sigmax dc beta],phitype,rp,dstate,iter,elast_init,itchat,chatter,bonddbond1,dtol,ndm);
                dddt2(1:2,k) = 1/hi*(Ri-dvec3);
            end
%         hhh = 1e-12;
% %         for j = 1:props.num_hard
%             for k = 1:2
...
%                 dvec,nvec,sigmax,Hc,dc,rp,dstate,iter,elast_init,itchat,chatter,bonddbond1,dtol,ndm);
            [dvec,dddt,dmax,dstate,bonddbond2,dbond,chatter,tvec] = GeneralCZM(tvtr,jumpu,dmax,dvec, ...
    nvec,[sigmax*dc/2 sigmax dc beta],phitype,rp,dstate,iter,elast_init,itchat,chatter,bonddbond1,dtol,ndm);
