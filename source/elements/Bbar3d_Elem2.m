...
%         Pdev = I4 - 1/3*OneOne;
        ...
        for l = 1:lint
                ...
                deps3d = Bbar*reshape(ul-ul_n,nst,1); %3D enhanced strain
%                 pn_1 = bulk*One'*eps3d;
                ephr = nh1-1+(l-1)*12; %pointer for plastic strain at pt l
                betahr = nh1-1+(l-1)*12+6;
                ahr = nh1-1+l*12;
                ...
                [sig_n1,Cmat,ee_n1,beta_n1,a_n1] = J2RadialReturnE(deps3d,ee_n,beta_n,a_n,mu,bulk,Khard,Hhard,sigy);
                
                % Retrieve deviatoric output
                sdev3 = sig_n1 - 1/3*OneOne*sig_n1;
                
                % Store history variables
                ephr = nh2-1+(l-1)*12; %pointer for plastic strain at pt l
                betahr = nh2-1+(l-1)*12+6;
                ahr = nh2-1+l*12;
                hr(ephr+1) = ee_n1(1);
                hr(ephr+2) = ee_n1(2);
                hr(ephr+3) = ee_n1(3);
