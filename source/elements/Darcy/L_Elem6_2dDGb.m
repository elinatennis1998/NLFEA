...
%%       
            ...
            bnAdN1w = gamL*nvect*[DmatL [1; 1; 0]]*diag([1 1 1 -1])*BmatL;
            bnAdN2w = gamR*nvect*[DmatR [1; 1; 0]]*diag([1 1 1 -1])*BmatR;
            bnAdNLw = nvect*[DmatL [1; 1; 0]]*diag([1 1 1 -1])*BmatL;
            bnAdNRw = nvect*[DmatR [1; 1; 0]]*diag([1 1 1 -1])*BmatR;
...
%             ElemFL = ElemFL - c1*( - NmatL(1:2,:)'*(tvtr + Kinv(1:2,1:2)*jump(1:2)) + bnAdN1w'*(jump(1:2)));
%             ElemFR = ElemFR - c1*( + NmatR(1:2,:)'*(tvtr + Kinv(1:2,1:2)*jump(1:2)) + bnAdN2w'*(jump(1:2)));
%         
...
%             jumpp = jump(3);
            ...
            % Not needed for linear problems
            ElemFL = ElemFL - c1*( - NmatL(1:2,:)'*(tvtr + Kinv(1:2,1:2)*jump(1:2)) + bnAdN1w'*(jump(1:2)) - 0*bnAdNLw'*(Kinvp*jumpflux));
            ElemFR = ElemFR - c1*( + NmatR(1:2,:)'*(tvtr + Kinv(1:2,1:2)*jump(1:2)) + bnAdN2w'*(jump(1:2)) + 0*bnAdNRw'*(Kinvp*jumpflux));
            ...
            ElemKLL = ElemKLL + c1*N1'*Kinv*N1 - 0*c1*bnAdNLw'*Kinvp*bnAdNL;
            ElemKLR = ElemKLR - c1*N1'*Kinv*N2 + 0*c1*bnAdNLw'*Kinvp*bnAdNR;
            ElemKRL = ElemKRL - c1*N2'*Kinv*N1 + 0*c1*bnAdNRw'*Kinvp*bnAdNL;
            ElemKRR = ElemKRR + c1*N2'*Kinv*N2 - 0*c1*bnAdNRw'*Kinvp*bnAdNR;
