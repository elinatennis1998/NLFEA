...
% mode shapes.
%
% Built/modified from NL_Elem2_1dRFB. Tested 7/4/14 only for linear
% elements. Verified to agree with the original routine for some test
% problems. Also, the condensed (CS) Kstar matrix agrees exactly with the
% Mstar matrix in NL_Elem5_1dEquiv.m for uniform meshes, where
% Mstar = 1/(1-tau)*Nbeta*tstep^2*Kstar.
%
% Runs either as a linear element type or nonlinear element type (nonlin=0
% or 0 in third row of MatTypeTable).
%
% This is the first implementation, which uses the average bubble which is
% calculated to be identical to Harari (formula for Kp and Mp). Later I
% will change it to be different values of the bubble at each integration
% point.
