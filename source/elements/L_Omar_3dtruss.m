...
%%
    case 3
        ...
        cz2= (diff(xl(3,:))/ElemL)^2; % cosz^2
        cxy= diff(xl(1,:))*diff(xl(2,:))/(ElemL^2); % cosx*cosy
        cxz= diff(xl(1,:))*diff(xl(3,:))/(ElemL^2); % cosx*cosz
        cyz=diff(xl(2,:))*diff(xl(3,:))/(ElemL^2); % cosy*cosz
        % Convert from local to global coordinates
        Local2Globalcoor=[ cx2,  cxy,  cxz, -cx2, -cxy, -cxz 
