...
% includes tau as a material parameter
% higher-order elements
% VMS form: M + k^2*K - t*(M - k^2*LLT + k^4*T) = (1-t)*F;
...
%         ElemF = zeros(ndf*nel,1);
        ...
        % Final formulas
        ElemM = ElemM + tau/(1-tau)*(ElemLLT + ElemK); % combined mass
        ElemK = ElemK - tau/(1-tau)*ElemT; % combined stiffness
        ElemF = -(ElemM + ElemK)*ul';
        ElemK = ElemM + ElemK;
...
%  ((x1 - x2)*(120*x1 + 240*x2 - 40*ksquar*x1 - 80*ksquar*x2 - 60*x1*x2 + 20*ksquar*x1^2 - 6*ksquar*x1^3 + 60*ksquar*x2^2 + ksquar*x1^4 - 24*ksquar*x2^3 + 5*ksquar*x2^4 - 30*x1^2 - 90*x2^2 - 18*ksquar*x1*x2^2 - 12*ksquar*x1^2*x2 + 4*ksquar*x1*x2^3 + 2*ksquar*x1^3*x2 + 3*ksquar*x1^2*x2^2 + 40*ksquar*x1*x2 - 240))/3];
        ElemF = [ -(12*ksquar*sin(pi*wave*x1) - 12*ksquar*sin(pi*wave*x2) + 240*pi^2*wave^2*x1^2 - 240*pi^2*wave^2*x1^3 + 240*pi^2*wave^2*x2^2 + 90*pi^2*wave^2*x1^4 - 120*pi^2*wave^2*x2^3 + 30*pi^2*wave^2*x2^4 + 12*pi^2*wave^2*sin(pi*wave*x1) - 12*pi^2*wave^2*sin(pi*wave*x2) + 80*pi^2*ksquar*wave^2*x1^3 - 60*pi^2*ksquar*wave^2*x1^4 + 40*pi^2*ksquar*wave^2*x2^3 + 24*pi^2*ksquar*wave^2*x1^5 - 20*pi^2*ksquar*wave^2*x2^4 - 5*pi^2*ksquar*wave^2*x1^6 + 6*pi^2*ksquar*wave^2*x2^5 - pi^2*ksquar*wave^2*x2^6 + 360*pi^2*wave^2*x1^2*x2 - 120*pi^2*wave^2*x1^3*x2 - 12*pi^3*wave^3*x1*cos(pi*wave*x1) + 12*pi^3*wave^3*x2*cos(pi*wave*x1) - 480*pi^2*wave^2*x1*x2 - 12*pi*ksquar*wave*x1*cos(pi*wave*x1) + 12*pi*ksquar*wave*x2*cos(pi*wave*x1) - 120*pi^2*ksquar*wave^2*x1^2*x2 + 80*pi^2*ksquar*wave^2*x1^3*x2 - 30*pi^2*ksquar*wave^2*x1^4*x2 + 6*pi^2*ksquar*wave^2*x1^5*x2)/(3*pi^2*wave^2*(x1 - x2))
