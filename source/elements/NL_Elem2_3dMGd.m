...
%             lint = 27;
 ...
 Bmat(:,4*mm-3:4*mm) = [Qxy(mm,1) 0         0         0
    ...
    %         [F,JxX,fi] = ikine2d(Qxy,ul,nel); %this is equivalent to above two lines
            ...
            rho_k0 = (1 + var_n) * rho0;
            facs= (rho/rho0)^expn;
            dres=  1/dt    - (expn-expm) * (rho/rho0)^(expn-expm) / rho *psi0_neo; % 1/dt - dR0/drho
            Rres = (rho-rho_k0)/dt - ((rho/rho0)^(expn-expm)*psi0_neo-psi0); % residual of mass-balance
...
[    sigma2(1),        0,        0,           sigma2(4)/2,                 0,           sigma2(6)/2,           sigma2(4)/2,                 0,          -sigma2(6)/2
  ...
  -sigma2(6)/2,        0,  sigma2(6)/2,          -sigma2(5)/4,           sigma2(4)/4, sigma2(1)/4 - sigma2(3)/4,          -sigma2(5)/4,          -sigma2(4)/4, sigma2(1)/4 + sigma2(3)/4];
            ...
            svec = -(rho/rho0)^(expn-expm)*sigma2; % dr0/dF
            svec2 = expn/rho*facs*sigma2; % dP/drho
            Dmat = [facs*Smat svec2
                    svec' dres];
            
            ElemF = ElemF - c1*(Bmat'*[facs*sigma2; Rres]);
