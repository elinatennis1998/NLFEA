...
%       real*8    v(3,3), d(3), a(3), b(3), z(3)
        d = zeros(3,1); a = zeros(3,1); b = zeros(3,1); z = zeros(3,1);
...
% c       Perform sweeps for rotations
        ...
        for i = 1:3
          ...
          aij  = a(i);
          g    = 100.d0*abs(aij);
          if(abs(d(i))+g~=abs(d(i)) || ...
             abs(d(j))+g~=abs(d(j))) %then
            ...
            if(abs(aij)>thresh) %then
              ...
              if(abs(h)+g==abs(h)) %then
                ...
                t = 2.d0*sign(h/aij)/(abs(h/aij)+sqrt(4.d0+(h/aij)^2));
              end %if
