function plotStreCont3(Coordinates, Contour, NodesOnElement, RegionOnElement, ProbType, MateT, ...
                       MatTypeTable, compon, PlotID, elemlist, ...
                       nodelist, MNE_ID, nelPn, varargin)
% Tim Truster
% 11/26/2013
% 
% Plot un-processed stress contour of finite element field from a 3-D mesh
% Optional arguments are in {}, defaults are in ().
%
% Default usage (for copy/paste):
% plotStreCont3(Coordinates,Contour,NodesOnElement,RegionOnElement,ProbType,MateT,MatTypeTable,compon,...
%               1,(1:size(NodesOnElement,1)),(1:size(Coordinates,1)),[1 0 0],[4 8 10 27 0])
%
% Inputs:
%  Coordinates = nodal coordinates [x y]
%  Contour = nodal solution field [c]
%  NodesOnElement = element connectivity [n1 n2 ... nnen]
%  RegionOnElement = element assigned material ID
%  ProbType, MateT, MatTypeTable - see input file discussion; note: the
%    material ID is assumed to be global, i.e. multi-materials are not
%    allowed
%  compon = stress component xx=1, yy=2, xy=3
...
switch nargin
    case {1,2,3,4,5,6,7}
        error('Must supply Coordinates, Contour, NodesOnElement, and RegionOnElement')
    case 8
        ModelID = 1;
        clfyn = 1;
        subp = 1;
...
for elemp = 1:length(elemlist)
    ...
    PatchPoints = Coordinates(ElemFlag,1:ndm)';
    ContourValues = Contour(ElemFlag,1:ndf)';
    ...
    if nelB == 4
        ...
        if ifflag
        fflag = 0;
        [itemo,indo,imatch] = binsearch(ElemFlag(1),nodelist,1,len);
        fflag = fflag + imatch;
        [itemo,indo,imatch] = binsearch(ElemFlag(2),nodelist,1,len);
        fflag = fflag + imatch;
        [itemo,indo,imatch] = binsearch(ElemFlag(3),nodelist,1,len);
        fflag = fflag + imatch;
        else
        fflag = 3;
        end
        ...
        if fflag == 3 
            ...
            ul = ContourValues;
            SurfOrientEtoS
            ul(1:ndf,1:nel) = ul(1:ndf,ilist);
            helem = plot3DLagransurfstre(xl,ul,nel,knots,lenn,'none',elemID,mateprop,compon,nelP,iel,elemp,helem);
        ...
        [itemo,indo,imatch] = binsearch(ElemFlag(3),nodelist,1,len);
        fflag = fflag + imatch;
        [itemo,indo,imatch] = binsearch(ElemFlag(4),nodelist,1,len);
        fflag = fflag + imatch;
        else
        fflag = 3;
        end
        ...
        if ifflag
        fflag = 0;
        [itemo,indo,imatch] = binsearch(ElemFlag(5),nodelist,1,len);
        fflag = fflag + imatch;
        [itemo,indo,imatch] = binsearch(ElemFlag(6),nodelist,1,len);
        fflag = fflag + imatch;
        [itemo,indo,imatch] = binsearch(ElemFlag(7),nodelist,1,len);
        fflag = fflag + imatch;
        [itemo,indo,imatch] = binsearch(ElemFlag(8),nodelist,1,len);
        fflag = fflag + imatch;
        else
        fflag = 4;
        end
...
if iscell(varargin{1,1}) % Multiple types of objects specified
    ...
    for obj = 1:numObj
        ...
        switch objtype
            ...
            case {'colorbar','CB','Colorbar','ColorBar'}
