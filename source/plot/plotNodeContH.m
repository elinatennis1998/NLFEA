function plotNodeContH(Coordinates, Contour, NodesOnElement, PlotID, elemlist, MNE_ID, varargin)
% Tim Truster
% 08/03/2014
% 
% Plot deformed shape of beam elements.
% Optional arguments are in {}, defaults are in ().
%
% Default usage (for copy/paste):
% plotNodeContH(Coordinates,Contour,NodesOnElement,1,(1:size(NodesOnElement,1)),[1 0 0])
%
% Inputs:
%  Coordinates = nodal coordinates [x]
...
%                varargin = {'cont'},{'Color',[1 0 0]} % changes contour
%                           color
...
for elemp = 1:length(elemlist)
    ...
    PatchPoints = Coordinates(NodesOnElement(elem,1:nel),1);
    ContourValues = Contour(NodesOnElement(elem,1:nel),1:2);
...
%     hold off
    ...
    len = 21;
    knots = (-1:.1:1.0)';
    hcont = plot1DLagranbeamcont(PatchPoints, ContourValues, nel, knots, len, elemp, hcont);
...
if iscell(varargin{1,1}) % Multiple types of objects specified
    ...
    for obj = 1:numObj
        ...
        switch objtype
            ...
            case {'cont','Cont','Contour','contour','field','Field'}
                contstuff = varargin{1,1+obj}; % get list of contour properties
                set(hcont,contstuff{:}) % set contour properties
        ...
        switch objtype
            ...
            case {'cont','Cont','Contour','contour','field','Field'}
                contstuff = varargin(1,2:end); % get list of contour properties
                set(hcont,contstuff{:}) % set contour properties
            case {'elem','Elem','elems','Elems','elements','Elements','elemtext','ElemText'}
                if elemyn
                elemstuff = varargin(1,2:end); % get list of elem text properties
