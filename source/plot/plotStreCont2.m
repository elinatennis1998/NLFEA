function plotStreCont2(Coordinates, Contour, NodesOnElement, RegionOnElement, ProbType, MateT, ...
                       MatTypeTable, compon, PlotID, elemlist, MNE_ID, ...
                       zcont3D, nelPn, varargin)
% Tim Truster
% 11/26/2013
% 
% Plot un-processed stress contour of finite element field from a 2-D mesh
% Optional arguments are in {}, defaults are in ().
%
% Default usage (for copy/paste):
% plotStreCont2(Coordinates,Contour,NodesOnElement,ProbType,MateT,MatTypeTable,compon,...
%               1,(1:size(NodesOnElement,1)),[1 0 0],0,[3 4 6 9 0])
%
% Inputs:
...
for elemp = 1:length(elemlist)
    ...
    PatchPoints = Coordinates(NodesOnElement(elem,1:nel),1:ndm);
    ContourValues = Contour(NodesOnElement(elem,1:nel),1:ndf);
...
%                  0 0 0 0.5  0.5  0.5 1.0  1.0  1.0]';
        ...
        helem = plot2DLagransurfstre(PatchPoints,ContourValues,nel,knots,len,'none',elemID,mateprop,compon,nelP,iel,zcont3D,elemp,helem);
