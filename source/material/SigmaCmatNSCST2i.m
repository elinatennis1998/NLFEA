function [sigma, D] = SigmaCmatNSCST2i(F,J,mateprop)
%
...
% Don't try to revert back to 4x4 calculations; keeping it as 6x6 makes it
% easier to go to 3D
...
sigma = [sigma([1,2,4]); 0];
D = [D([1,2,4],[1,2,4]) zeros(3,1); zeros(1,4)];
...
matp = 1/2*matE - 1/3*mat1;
            ...
            D = -2/3*(one*sigma' + sigma*one' - mu*trb*matp);
...
% mu1/2=c1, mu2/2=c2, mu=mu1+mu2=2*(c1+c2) where mu is the small-strain shear modulus
        ...
        J23 = J^(-2/3);
        b = J23*(F*F');
        bv = [b(1,1); b(2,2); b(1,2); 0];
        b1 = b*b;
        b2 = [b1(1,1); b1(2,2); b1(1,2); 0];
        b3 = (bv*bv');
        b4 = [2*bv(1)*bv(1) 2*bv(3)*bv(3) 2*bv(1)*bv(3) 0
              2*bv(3)*bv(3) 2*bv(2)*bv(2) 2*bv(2)*bv(3) 0
              2*bv(1)*bv(3) 2*bv(2)*bv(3) (bv(3)*bv(3)+bv(1)*bv(2)) 0
              0 0 0 0];
        I1 = trace(b) + J23;
        I2 = 1/2*(I1^2 - (trace(b1) + J23^2));
%         J43 = J^(-4/3);

        sigma1 = 2*c1*(bv - 1/3*I1*one);
        sigma2 = 2*c2*(I1*bv - b2 - 2/3*I2*one);

        sigma = sigma1 + sigma2;
        
        D1 = -2/3*(one*sigma1' + sigma1*one' + 2*c1*I1*(1/3*mat1 - 1/2*matE));
        D2 = -4/3*(one*sigma2' + sigma2*one') + 4*c2*(b3 - 1/2*b4 - 4/9*I2*mat1 + 1/3*I2*matE);
        ...
        bp = sqrt(diag(bp));
        Q = m';
        P = [Q(1,1)^2 Q(1,2)^2 2*Q(1,1)*Q(1,2)
             Q(2,1)^2 Q(2,2)^2 2*Q(2,1)*Q(2,2)
             Q(1,1)*Q(2,1) Q(1,2)*Q(2,2) Q(1,1)*Q(2,2)+Q(1,2)*Q(2,1)];
        sig = zeros(3,1);
        for i = 1:2
            si = 0;
            for r = 1:n
        ...
        C2 = C(2);
        if bp(1) ~= bp(2)
        C3 = (bp(2)^2*sig(1) - bp(1)^2*sig(2))/(bp(1)^2-bp(2)^2);
        else
        C3 = 1/2*C1;
        end

        Diso = P*diag([C1,C2,C3])*P';
        Diso = [Diso zeros(3,1); zeros(1,4)];
...
matE = diag([2,2,2,1,1,1]);
        ...
        b = F*F';
        bv = [b(1,1); b(2,2); 1; b(1,2); 0; 0];
        Ib = [2*bv(1)*bv(1) 2*bv(4)*bv(4) 2*bv(6)*bv(6) 2*bv(1)*bv(4) 2*bv(4)*bv(6) 2*bv(6)*bv(1)
              2*bv(4)*bv(4) 2*bv(2)*bv(2) 2*bv(5)*bv(5) 2*bv(2)*bv(4) 2*bv(5)*bv(2) 2*bv(4)*bv(5)
              2*bv(6)*bv(6) 2*bv(5)*bv(5) 2*bv(3)*bv(3) 2*bv(6)*bv(5) 2*bv(5)*bv(3) 2*bv(3)*bv(6)
              2*bv(1)*bv(4) 2*bv(2)*bv(4) 2*bv(6)*bv(5) (bv(1)*bv(2)+bv(4)*bv(4)) (bv(4)*bv(5)+bv(6)*bv(2)) (bv(6)*bv(4)+bv(1)*bv(5))
              2*bv(4)*bv(6) 2*bv(5)*bv(2) 2*bv(5)*bv(3) (bv(4)*bv(5)+bv(6)*bv(2)) (bv(2)*bv(3)+bv(5)*bv(5)) (bv(5)*bv(6)+bv(4)*bv(3))
              2*bv(6)*bv(1) 2*bv(4)*bv(5) 2*bv(3)*bv(6) (bv(6)*bv(4)+bv(1)*bv(5)) (bv(5)*bv(6)+bv(4)*bv(3)) (bv(3)*bv(1)+bv(6)*bv(6))];
        [m,bp] = eig(b);
        bp = [sqrt(diag(bp)); 1];
        bpt = bp/J^(1/3);
        I1 = trace(b)+1;
        ...
        mmat = [m(1,1)^2      m(1,2)^2      0
                m(2,1)^2      m(2,2)^2      0
                0             0             1
                m(1,1)*m(2,1) m(1,2)*m(2,2) 0
                0             0             0
                0             0             0];
        ...
        dgm2 = 1/DA(2)*(Ib/2-(bv*bv')+I3*bp(2)^-2*(mat1-matE/2)) ...
            ...
            sigma = bA(1)*one + (bA(2) - bA(1))*mmat(:,2);
...
% % Original version: all material models debuged 11/2/2011
% function [sigma, D] = SigmaCmatNSCST2i(F,J,mateprop)
% %
% % Shear constitutive material model evaluation for NL elasticity
% % 10/16/2011
...
%             C1b = C1 + 2*C2*(I1-3) + 3*C3*(I1-3)^2;
% 
%             sigma = J23*2*C1b*(bv - 1/3*trb*one);
%             sigma2 = J23*2*C1b*matp*bv2;
%             
%             D = -2/3*(one2*sigma2' + sigma2*one2' - 2*C1b*J23*trb*matp) ...
%                 + 8*J43*(C2 + 3*C3*(I1-3))*matp*(bv2*bv2')*matp;
%             
%             D = [D([1,2,4],[1,2,4]) zeros(3,1); zeros(1,4)];
%             
%         else % mati == 6 % Anisotropic, Stein CMAME190
...
%             C5 = 2/3*C4b;
%             C6 = 8*J43*C4;
%             av = [aa(1,1); aa(2,2); aa(1,2); 0];
%             tra = trace(aa);
%             av2 = [aa(1,1); aa(2,2); 0; 2*aa(1,2); 0; 0];
% 
%             sigma = J23*2*C1b*(bv - 1/3*trb*one) + C4b*(av - 1/3*tra*one);
%             sigma2 = J23*2*C1b*matp*bv2;% + C4b*matp*av2;
%             
%             D = -2/3*(one2*sigma2' + sigma2*one2' - 2*C1b*J23*trb*matp) ...
%                 + 8*J43*(C2 + 3*C3*(I1-3))*matp*(bv2*bv2')*matp ...
%                 + C5*(tra*matp-(one2*av2'*matp + matp*av2*one2')) ...
%                 + C6*(matp*(av2*av2')*matp);
%             
%             D = [D([1,2,4],[1,2,4]) zeros(3,1); zeros(1,4)];
%             
%         end
