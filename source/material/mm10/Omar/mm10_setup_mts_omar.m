...
function [props, np1, n] = mm10_setup_mts_omar(props, np1, n)
%       use mm10_defs
% init_hard = ?;
%       implicit none
% c
...
% c     Same here -- check for previous step flag
      if (n.tau_tilde(1) < 0.0)% then
        n.tau_tilde(1) = props.tau_a + (np1.mu_harden/props.mu_0)*np1.tau_y...
            + init_hard;
      end
      np1.backstress_omar = n.backstress_omar;
% c     
%       return
end