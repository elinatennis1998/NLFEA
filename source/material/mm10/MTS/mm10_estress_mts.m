...
function [props, np1, n, stress, tt, et] = mm10_estress_mts(props,...
    np1, n, stress, tt)
%       use mm10_defs
%       implicit none
% c
...
%       integer :: i
      ...
      etmat = (ones(6,1)*((ct+np1.tau_l'/cta).^(props.voche_m).*abs(stress*np1.ms).^(props.rate_n-2.0).*(stress*np1.ms))).*np1.ms;
