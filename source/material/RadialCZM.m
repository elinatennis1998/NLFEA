function [dvec,dddt,dmax,dstate,bonddbond2,dbond,chatter,tvec] = RadialCZM(tvtr,jumpu,dmax,dvec, ...
...
if tn >= 0 % tension
 ...
 if iter > itchat && chatter == 1
  ...
  elseif dmax > 0 && rp*dmax <= normtvec && normtvec < rp*dmax + psik && (dstate ~= 4 && iter == 0) %unloading
    ...
    dvec = dmax*ntilda;
    dddt = dmax/normtvec*(eye(ndm) - ntilda*ntilda');
    dddu = rp*dddt;
%     deq = sqrt(dvec'*dvec);
%     dmax = max(dmax,deq);
    dstate = 3;
...
%                 dmax = max(dmax,deq);
 ...
 if iter > itchat && chatter == 1
  ...
  elseif dmax > 0 && rp*dmax <= normtvec && normtvec < rp*dmax + psik && (dstate ~= 4 && iter == 0) %unloading
    ...
    ntilda = tvec/normtvec;
    dvec = dmax*ntilda;
    dddt = dmax/normtvec*(eye(ndm) - ntilda*ntilda' - nvec*nvec');
    dddu = rp*dddt;
%     deq = sqrt(dvec'*dvec);
%     dmax = max(dmax,deq);
