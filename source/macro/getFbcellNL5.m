function eF = getFbcellNL5(xl,d,xe,ue,ndf,ndm,nel,nen,nummat,strong,MR,BR,MS,BS,sslot,tractyn,lamda)
%
...
%.....---------------------------------------------------
         for l=1:4
            ...
            if(nel==3||nel==6)
              ...
              [shl,shls,be] = shlt(litr,lits,nel,nel,der,bf);
              [shg, shgs, det, be, sx] = shgt(xl,nel,shl,shls,nen,bf,der,be);
            else
              [w,litr,lits] =  intpntq(l,4,ib);
              [shl,shls,be] = shlq(litr,lits,nel,nel,der,bf);
              [shg, shgs, det, be, sx] = shgq(xl,nel,shl,shls,nen,bf,der,be);
            end
...
% Compute element shape functions Nbar(R,S)
            ...
            if(nel==3||nel==6)
              ...
              [she] = shgt(xe,nel,shel,shels,nen,bf,der,bubble);
            else
              [shel,shels,bubble] = shlq(bigR,bigS,nel,nel,der,bf);
              [she] = shgq(xe,nel,shel,shels,nen,bf,der,bubble);
            end
...
% Compute element shape functions Nbar(R,S)
            ...
            if(nel==3||nel==6)
              [shel] = shlt(bigR,bigS,nel,nel,der,bf);%,shed,shels,bubble
%               [she] = shgt(xe,nel,shed,shels,nen,bf,der,bubble);
            else
              [shel] = shlq(bigR,bigS,nel,nel,der,bf);%,shed,shels,bubble
