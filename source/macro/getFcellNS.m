function eF = getFcellNS(xc,uc,d,iprob,xl,ul,ndf,ndfs,ndm,lint,nel,nen,nummat,MR,BR,MS,BS,t11,t12,t21,t22)
%
...
%               WRONG -> fixed it on 3/15/12 to give correct errors for
%               Cook's membrane compared to small deformations
              [shg, shgs, det] = shgt(xc,nel,shld,shls,nen,bf,der,be);
            else
              [w,litr,lits] =  intpntq(l,lint,ib);
              [shl,shld,shls,be] = shlq(litr,lits,nel,nel,der,bf);
%               [shg, shgs, det] = shgq(xl,nel,shld,shls,nen,bf,der,be);
              [shg, shgs, det] = shgq(xc,nel,shld,shls,nen,bf,der,be);
            end
...
% Compute element shape functions Nbar(R,S)
            ...
            sigmap = press*spvec;
            cmatp = press*cpmat;
            [theta1,theta2,theta3] = ThetaNS(JxX,d);
