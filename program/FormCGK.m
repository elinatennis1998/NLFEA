...
switch isw%Task Switch
    ...
    case 3 %Get Stiffness, Force
        Kddstore = sparse(Ksdd(:,1),Ksdd(:,2),0,neq,neq);
        Kdfstore = sparse(Ksdf(:,1),Ksdf(:,2),0,neq,nieq);
        Kfdstore = sparse(Ksdf(:,2),Ksdf(:,1),0,nieq,neq);
        Kffstore = sparse(Ksff(:,1),Ksff(:,2),0,nieq,nieq);
        AssemQuant = 'AssemStifForcG';
        Fd1store = Fext1;
        Fd3store = zeros(nieq,1);
    case 6 %Get Force
        hflgu = 1;
        h3flgu = 1;
...
for gid = 1:length(RegList)
    
    grain = RegList(gid);
    grainG1 = RegionOnElement(RegionOnElement==grain);
